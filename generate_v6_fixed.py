...
# 封面单独生成
cover_path = generate_cover_v6(
    'Claude Code\n深度解析',
    '基于 51.2 万行源码的深度技术解析',
    'd:/work/项目/XHS-AI-GitHub-每日创作助手/Claude_Code_Deep_v6_cover.png'
)
...
pages = [
    {
        ...
        'explanation': '传统AI助手采用"生成-执行"串行模式，Claude Code 的创新在于流式推理：当 LLM 生成工具调用时立即执行，无需等待完整响应。这种设计将延迟从"完整生成时间+执行时间"降低到"首 token 时间+增量执行时间"，实现真正的交互式编程体验。'
    },
    {
        'title': '多智能体：三层协作架构',
        ...
        'explanation': 'Claude Code 的多智能体系统通过"分叉-并行-合并"模式实现高效协作。主实例通过 KV Cache 分叉创建子实例，各实例并行工作，共享主实例的上下文。Git Worktree 技术确保多个智能体不会产生文件冲突，实现真正的并行编程辅助。'
    },
    {
        'title': '工具系统：40+ 工具的编排',
        'image_type': 'tools',
    ...
    },
    {
        ...
        'explanation': 'Claude Code 的安全模型采用"最小权限原则"：默认状态下只允许读取文件，修改需要显式授权。命令执行在沙箱中进行，危险命令被加入黑名单。提示注入检测机制防止恶意代码利用 AI 传播，安全机制贯穿从系统策略到运行时监控的每个层面。'
    },
    {
...
paths = generate_multi_page_v6(
    ...
    '基于 51.2 万行源码的深度技术解析',
    pages,
    'd:/work/项目/XHS-AI-GitHub-每日创作助手'
)
